...
function TableRegion({ setOpen, setRegion, searchObject, region }) {
    const { regions, update, setUpdate } = useContext(ContextRegions)
    ...
    const filteredRegions = regions.filter((region) =>
        region.name.toLowerCase().includes(searchObject.toLowerCase())
    );

    // Xử lý các hàng hiển thị trong một trang
    const rowsToDisplay = filteredRegions?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);

    const handleDelete = async () => {
        if (region.imgUrl && region.imgUrl.includes('cloudinary.com')) {
            // Lấy `public_id` từ URL của Cloudinary
            const publicId = region.imgUrl
                .split('/').slice(-2).join('/')  // Lấy thư mục và tên file từ URL
                .replace(/\.[^/.]+$/, '');       // Loại bỏ phần mở rộng file (ví dụ: .jpg, .png)
            await deleteImageFromCloudinary(publicId);
        }

        await axios.delete(`http://localhost:8080/api/regions/${region.id}`);
        setUpdate(!update);
                ...
                <Table sx={{ minWidth: 650 }} aria-label="simple table">
                    <TableHead sx={{ backgroundColor: '#1F2937' }}>
                        <TableRow sx={{ whiteSpace: 'nowrap'}}>
                            ...
                            <TableCell align="center" sx={{ color: 'white' }}>Region Name</TableCell>
                            ...
                            >
                                ...
                                <TableCell align="center">
                                    <div className="flex justify-center items-center">
                                        <Button
                                            onClick={() => { setOpen(true); setRegion(row) }}
                                            sx={{
                                        ...
                                        <Button
                                            onClick={() => {
                                                setOpenDeleted(true); setRegion(row);
                                            }}
                                            sx={{
                                                marginLeft: "10px",
