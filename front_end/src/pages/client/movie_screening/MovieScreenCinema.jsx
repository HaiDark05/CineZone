...
function MovieScreenCinema(props) {
    ...
    const { setBooking , inner } = useContext(ContextBooking);
            ...
            <div className="m-auto p-8 w-[80%] grid md:grid-cols-2 gap-6">
                {cinemaShow.length > 0 ? (
                    cinemaShow.map((cinema, index) => {
                        ...
                        return (
                            <div key={index} className="border rounded-lg overflow-hidden shadow-lg bg-gray-800 text-white p-4">
                                {hasInfo && (
                                    <div>
                                        <div className="w-full h-64">
                                            <img
                                                src={cinema.imgUrl}
                                                alt=""
                                                className="w-full h-64 object-cover transition-transform duration-300 hover:scale-105 hover:opacity-90"
                                            />
                                        </div>
                                        <div className="p-4">
                                            ...
                                            <Link to={`/moviescreening/${cinema.id}`}>
                                                <button onClick={() => setBooking(inner)} className="mt-3 bg-yellow-400 text-black px-4 py-2 rounded font-bold hover:bg-yellow-500">
                                                    XEM CHI TIẾT
                                                </button>
                                            </Link>
                                        </div>
